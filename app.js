...
app.post('/register', (req, res) => {
    ...
    fs.readFile(usersFile, (err, data) => {
        ...
        if (users.some(u => u.username === username)) {
            return res.send(renderLayout('', "Brugernavnet findes allerede."));
        }
        bcrypt.hash(password, 10, (err, hash) => {
            if (err) {
                return res.send(renderLayout('', "Noget gik galt under oprettelsen."));
            }
            users.push({ username, password: hash });
            fs.writeFile(usersFile, JSON.stringify(users, null, 2), (err) => {
                if (err) {
                    return res.send(renderLayout('', "Kunne ikke gemme brugeren. Prøv igen."));
                }
                // Her viser vi en klar succesbesked
                res.send(renderLayout(`
                    <h1>Bruger oprettet!</h1>
                    <p>Du kan nu <a href="/login" class="btn btn-green">logge ind</a>.</p>
                `));
            });
        });
    });
